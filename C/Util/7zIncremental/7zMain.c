  ...
  if (res == SZ_OK)
  {
    ...
    if (res == SZ_OK)
    {
      ...
      */
      ...
      outBufferSize = 2048;
      outBuffer = (Byte *)SzAlloc(NULL, outBufferSize);
      if (outBuffer == NULL)
	{
	  res = SZ_ERROR_MEM;
	  goto exit1;
      ...
      for (i = 0; i < db.db.NumFiles; i++)
      {
        ...
        {
          res = SzArEx_Extract_Buffered(&db, &lookStream.s, i,
	  ...
	  do
	    {
	      ...
	      res = SzArEx_Extract_Buffered_Next(state, &blocksize);
	      if (res != SZ_OK)
		break;
	      if (blocksize == 0)
		break;

	      if (File_Write(&outFile, outBuffer, &blocksize) != 0)
		{
